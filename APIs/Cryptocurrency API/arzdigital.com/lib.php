...
function get_target_toman_price($target, $CONFIG, $allCurrencies) {
    $target = strtolower(trim($target));
    if ($target === 'usd') {
        return get_usd_to_toman_rate($CONFIG);
    }
...
function convert_price_to_target($price_toman, $tocurrency, $CONFIG, $allCurrencies) {
    $tocurrency = strtolower(trim($tocurrency));
    if ($tocurrency === '' || $tocurrency === 't' || $tocurrency === 'toman') {
        return ['price' => $price_toman, 'unit' => 'toman'];
    }
    if ($price_toman === null) {
        return ['price' => null, 'unit' => $tocurrency];
    }
    $targetTomanPrice = get_target_toman_price($tocurrency, $CONFIG, $allCurrencies);
    if ($targetTomanPrice && $targetTomanPrice > 0) {
        return ['price' => $price_toman / $targetTomanPrice, 'unit' => $tocurrency];
    }
    return ['price' => $price_toman, 'unit' => 'toman'];
}
...
function fetch_html($url, $connectTimeout, $timeout, $CONFIG = null) {
...
function persian_number_to_float($str) {
...
function extract_price_from_page($html, $type) {
    $digit = '[0-9۰-۹]';
    $num   = $digit . '+(?:[,،]' . $digit . '{3})*(?:\.' . $digit . '+)?';

    if (preg_match('/معادل\s*(' . $num . ')\s*تومان\s*است/u', $html, $m)) {
        return persian_number_to_float($m[1]);
    }
    if ($type === 'fiat' && preg_match('/(' . $num . ')\s*تومان/u', $html, $m)) {
        return persian_number_to_float($m[1]);
    }
    return null;
}
...
function extract_meta_content($html, $attr, $value) {
    if (preg_match_all('/<meta\s+[^>]*>/i', $html, $matches)) {
        foreach ($matches[0] as $tag) {
            if (preg_match('/' . preg_quote($attr, '/') . '\s*=\s*["\']' . preg_quote($value, '/') . '["\']/i', $tag)) {
...
function coin_icon_candidates($conf, $html) {
    $candidates = [];
    $slug = slug_from_url($conf['url']);
    if ($slug) {
        $candidates[] = 'https://cdn.arz.digital/cr-odin/img/assets/' . $slug . '/200x200.png';
    }
    if ($html && preg_match('/<img[^>]+class=["\'][^"\']*arz-coin-page-data__coin-icon[^"\']*["\'][^>]*src=["\']([^"\']+)["\']/i', $html, $m)) {
        $candidates[] = resolve_url($m[1], $conf['url']);
    }
    if ($html) {
        $og = extract_meta_content($html, 'property', 'og:image');
        if ($og) $candidates[] = resolve_url($og, $conf['url']);
    }
    $candidates[] = 'https://arzdigital.com/wp-content/themes/arz-theme/assets/img/default-coin.png';
    return array_values(array_unique(array_filter($candidates)));
}

function extract_title_label($html) {
    if (preg_match('/<h1[^>]*>([^<]{1,120})<\/h1>/iu', $html, $m)) {
        $label = trim(html_entity_decode(strip_tags($m[1]), ENT_QUOTES));
        if ($label !== '') return $label;
    }
    if (preg_match('/<title>([^<]{1,150})<\/title>/iu', $html, $m)) {
        $label = trim(html_entity_decode(strip_tags($m[1]), ENT_QUOTES));
        if ($label !== '') return $label;
    }
    return null;
...
function resolve_currency_conf($key, $CONFIG) {
    ...
    $url = sprintf($CONFIG['coin_url_pattern'], $slug);
    $html = fetch_html($url, $CONFIG['fetch_connect_timeout'], $CONFIG['fetch_timeout'], $CONFIG);
    if (!$html) {
        release_lock($slot);
        dynamic_lookup_negative_cache_set($slug, (int) ($CONFIG['negative_cache_seconds'] ?? 600), $CONFIG);
        return null;
    }
    $price = extract_price_from_page($html, 'crypto');
    if ($price === null) {
        release_lock($slot);
        dynamic_lookup_negative_cache_set($slug, (int) ($CONFIG['negative_cache_seconds'] ?? 600), $CONFIG);
        return null;
    ...
    release_lock($slot);
    $conf = [
        'label' => extract_title_label($html) ?? strtoupper($slug),
        'type'  => 'crypto',
        'url'   => $url,
    ];
    $registry[$slug] = $conf;
...
function fetch_html_batch($urlsByKey, $CONFIG, $maxConcurrent, $deadline = null) {
    $results = [];
    $queue = $urlsByKey;
    $active = [];
    $mh = curl_multi_init();

    $spawn = function ($key, $url) use (&$active, $mh, $CONFIG) {
        $ch = curl_init($url);
        curl_setopt_array($ch, [
            CURLOPT_RETURNTRANSFER  => true,
            CURLOPT_FOLLOWLOCATION  => true,
            CURLOPT_MAXREDIRS       => 3,
            CURLOPT_CONNECTTIMEOUT  => $CONFIG['fetch_connect_timeout'],
            CURLOPT_TIMEOUT         => $CONFIG['fetch_timeout'],
            CURLOPT_SSL_VERIFYPEER  => true,
            CURLOPT_SSL_VERIFYHOST  => 2,
            CURLOPT_IPRESOLVE       => CURL_IPRESOLVE_V4,
            CURLOPT_PROTOCOLS       => CURLPROTO_HTTPS,
    ...
    foreach ($urlsByKey as $key => $url) {
        if (count($active) >= $maxConcurrent) break;
        $spawn($key, $url);
        unset($queue[$key]);
    }
    ...
    do {
        ...
        while ($info = curl_multi_info_read($mh)) {
            ...
            $code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
            $err  = curl_error($ch);
            $results[$entry['key']] = ($html !== false && $html !== '' && !$err && $code < 400) ? $html : null;

            curl_multi_remove_handle($mh, $ch);
            curl_close($ch);
            unset($active[$id]);

            if ($queue && ($deadline === null || microtime(true) < $deadline)) {
                $nextKey = array_key_first($queue);
                $nextUrl = $queue[$nextKey];
                unset($queue[$nextKey]);
                $spawn($nextKey, $nextUrl);
            }
        }
...
function fetch_snapshots_batch($keysConf, $CONFIG, $deadline = null) {
    ...
    $maxConcurrent = max(1, (int) ($CONFIG['max_concurrent_fetches'] ?? 6));
    $pending = $urls;
    $htmlByKey = [];
    ...
    for ($try = 1; $try <= $attempts; $try++) {
        ...
        $batchResults = fetch_html_batch($pending, $CONFIG, $maxConcurrent, $deadline);
        $nextPending = [];
        foreach ($batchResults as $key => $html) {
            if ($html !== null) {
                $htmlByKey[$key] = $html;
            } else {
                $nextPending[$key] = $pending[$key];
            }
        }
        $pending = $nextPending;

        if ($pending && $try < $attempts && ($deadline === null || microtime(true) < $deadline)) {
            usleep((int) (($CONFIG['fetch_retry_delay_ms'] ?? 300) * 1000));
        }
    }

    $snapshots = [];
    foreach ($keysConf as $key => $conf) {
        if (!isset($htmlByKey[$key])) {
            $snapshots[$key] = null;
            continue;
        }
        $price = extract_price_from_page($htmlByKey[$key], $conf['type']);
        if ($price === null) {
            $snapshots[$key] = null;
            continue;
        }
        $snapshots[$key] = [
            'price'           => $price,
            'icon_candidates' => coin_icon_candidates($conf, $htmlByKey[$key]),
        ];
    }
    return $snapshots;
}
...
function fetch_currency_snapshot($conf, $CONFIG) {
    ...
    for ($i = 1; $i <= $attempts; $i++) {
        $html = fetch_html($conf['url'], $CONFIG['fetch_connect_timeout'], $CONFIG['fetch_timeout'], $CONFIG);
        if ($html) {
            $price = extract_price_from_page($html, $conf['type']);
            if ($price !== null) {
                return [
                    'price'           => $price,
                    'icon_candidates' => coin_icon_candidates($conf, $html),
                ];
            }
        }
        if ($i < $attempts) {
...
function update_history($existingRecord, $price, $historyDays) {
    $now = time();
    $history = (is_array($existingRecord) && isset($existingRecord['history'])) ? $existingRecord['history'] : [];
    $history[] = ['t' => $now, 'p' => $price];

    $cutoff = $now - ($historyDays * 86400);
    $history = array_values(array_filter($history, fn($pt) => $pt['t'] >= $cutoff));
...
function generate_chart($key, $label, $price, $candles, $changePct, $width, $height, $chartsDir, $fontsDir, $iconPath = null, $periodLabel = '24h', $unit = 'toman') {
    $unitDisplay = ($unit === 'toman') ? 'Toman' : strtoupper($unit);
    ...
    $pairText = strtoupper($label) . ' / ' . $unitDisplay;
    ...
    draw_text($img, $fontMedium, 18 * $scale, $unitX, $unitY, $gray, $unitDisplay);
...
function find_alias_snapshot($key, $conf, $currencies, $CONFIG) {
    ...
    foreach ($currencies as $otherKey => $otherConf) {
        ...
        if (($otherConf['url'] ?? null) !== $conf['url']) continue;
        ...
        return ['price' => $record['price'], 'icon_path' => $iconPath];
    }
    return null;
}

function finalize_price_record($key, $conf, $CONFIG, $existingRecord, $price, $iconPath) {
    $history = update_history($existingRecord, $price, $CONFIG['history_days']);

    $ref24h = price_at_or_before($history, time() - 86400);
    $change24h = ($ref24h && $ref24h > 0) ? (($price - $ref24h) / $ref24h) * 100 : 0.0;

    $ref2d = price_n_days_ago($history, 2);
    $change2d = ($ref2d && $ref2d > 0) ? (($price - $ref2d) / $ref2d) * 100 : $change24h;
...
function fetch_and_store_price($key, $conf, $CONFIG, $existingRecord, $currencies = null) {
    ...
    if ($alias !== null) {
        return finalize_price_record($key, $conf, $CONFIG, $existingRecord, $alias['price'], $alias['icon_path']);
    }
    ...
    return finalize_price_record($key, $conf, $CONFIG, $existingRecord, $snapshot['price'], $iconPath);
}
...
function fetch_and_store_prices_batch($items, $CONFIG, $currencies = null, $deadline = null) {
    ...
    foreach ($items as $key => $item) {
        ...
        $alias = find_alias_snapshot($key, $conf, $currencies, $CONFIG);
        if ($alias !== null) {
            $results[$key] = finalize_price_record($key, $conf, $CONFIG, $item['existing'], $alias['price'], $alias['icon_path']);
        } else {
            $needFetch[$key] = $conf;
        }
    }
    ...
    if ($needFetch) {
        ...
        foreach ($needFetch as $key => $conf) {
            ...
            $iconPath = ensure_icon($key, $snapshot['icon_candidates'], $CONFIG['paths']['icons'], $CONFIG);
            $results[$key] = finalize_price_record($key, $conf, $CONFIG, $existing, $snapshot['price'], $iconPath);
        }
    }
